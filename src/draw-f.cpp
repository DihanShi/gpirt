...
inline void ess_sparse_threadsafe(arma::vec& out, const arma::vec& f, const arma::vec& y, 
    ...
    while (reject) {
        ...
        for (arma::uword i = 0; i < n; ++i) {
            ws.f_prime(i) = f(i) * cos_eps + ws.nu(i) * sin_eps;
        }
...
void draw_f(arma::cube& result, const arma::cube& f, const arma::mat& theta, const arma::cube& y, 
    ...
    if (constant_IRF == 0) {
        ...
        // Pre-allocate combined matrices once
        ...
        for (arma::uword h = 0; h < horizon; h++) {
            arma::uword start_idx = h * n;
            arma::uword end_idx = (h + 1) * n - 1;
            theta_constant.subvec(start_idx, end_idx) = theta.col(h);
            for (arma::uword j = 0; j < m; ++j) {
                f_constant.col(j).subvec(start_idx, end_idx) = f.slice(h).col(j);
                y_constant.col(j).subvec(start_idx, end_idx) = y.slice(h).col(j);
                mu_constant.col(j).subvec(start_idx, end_idx) = mu.slice(h).col(j);
            }
        }
        ...
        for (arma::uword h = 0; h < horizon; ++h) {
            ...
            arma::uword end_idx = (h + 1) * n - 1;
            for (arma::uword j = 0; j < m; ++j) {
                result.slice(h).col(j) = f_prime.col(j).subvec(start_idx, end_idx);
            }
        }
    }
}